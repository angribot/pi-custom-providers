...
We deliberately ignore `allowNetwork: false` for exactly one attempt: if a credential exists, `PI_OFFLINE` is unset, and nothing is stored yet, we fetch anyway and share that single in-flight promise with concurrent or superseding cache-only generations. This violates the host's cache-only contract, so it is bounded — one attempt per process, skipped entirely once a Catalog is stored. The shared request uses its own 5s timeout rather than the first generation's abort signal; every generation still checks its signal and publishes through the host transaction, so only the current generation can persist or expose the result.
...
Forced refreshes are not deduplicated inside the extension. Pi owns their generation and cancellation semantics: a newer refresh for the same Provider ID aborts the older generation, and only the current generation can publish. Tests therefore require the replacement to win, not both requests to run to completion.
