...
The TUI status item is registered under one key and cleared on shutdown and whenever the selected model can't use Fast Mode, so a stale `⚡` never outlives the condition that produced it.
